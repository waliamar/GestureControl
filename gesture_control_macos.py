"""
Gesture Control System for macOS
...
class Config:
    ...
    VOLUME_STEPS_PER_ACTION = 3  # macOS volume adjustment
...
# =============================================================================
# SYSTEM CONTROL (macOS - AppleScript)
# =============================================================================

def send_media_key(key_name: str) -> None:
    """Send media key using AppleScript"""
    import subprocess
...
def open_spotify() -> None:
    """Open or focus Spotify application"""
    import subprocess
...
def main():
    ...
    with mp_hands.Hands(
        ...
        print("Gesture Control Active (macOS) - Press 'q' to quit")
                ...
                if is_rock_sign(landmarks):
                    ...
                    if abs(delta_y) > Config.VOLUME_STEP_PIXELS and \
                       (current_time - last_volume_time) > Config.VOLUME_REPEAT_DELAY:
                        ...
                        if delta_y < 0:
                            for _ in range(Config.VOLUME_STEPS_PER_ACTION):
                        ...
                        if elapsed_time <= Config.SWIPE_MAX_DURATION and \
                           abs(delta_x) >= Config.SWIPE_MIN_PIXELS:
                            ...
                            if swipe_gesture_type == "POINT":
                                ...
                                if delta_x > 0 and can_execute_action("cmd_tab"):
                                    send_key_combination("cmd+tab")
                                    mark_action_executed("cmd_tab")
                                    hud_messages.append("SYS: Next App")
                                elif delta_x < 0 and can_execute_action("cmd_shift_tab"):
                                    send_key_combination("cmd+shift+tab")
                                    mark_action_executed("cmd_shift_tab")
                                    hud_messages.append("SYS: Previous App")
